...
class Printer(object):
    ...
    def __str__(self) -> str:
        return "{} {} (s/n: {}) [USB dev {} / Bus {}]".format(self.vendor_name, self.product_name, self.serial_number,
                                                              self.usb_dev.address, self.usb_dev.bus)
...
def find_printers() -> List[Printer]:
    ...
    for dev in devs:
        supported_dev = next(
            filter(lambda x: x.vendor_id == dev.idVendor and x.product_id == dev.idProduct, SUPPORTED_DEVICES)
            , None)
